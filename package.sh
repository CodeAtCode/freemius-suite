...
if [ -f "$output"/"$packagename"-"$version".zip ]; then
    echo "-Package already exists, ignore the package generation"
    slack-message "Package uploading for $packagename at $version in progress..."
else
